...
- `data_rna_seq_mrna.txt` – Expression data file from LENS
    - Generated by: `lens_files.main()`

- `data_gene_signatures.txt` – Generic assay file from LENS
    - Generated by: `lens_files.main()`

- `meta_rna_seq_mrna.txt` – Metadata file for expression data file
    - Generated by: `lens_files.main()`

- `meta_gene_signatures.txt` – Metadata file for generic assay file
    - Generated by: `lens_files.main()`
...
- `<filename>_profile_report.html` - summary report containing summary statistics like duplicates, row count using `ydata-profiling` tool (see their docs for more information how what is featured: https://docs.profiling.ydata.ai/latest/) generated for each fileformat available for the dataset
...
3. Run `lens_files.py` to download the lens files from Synapse for validation later
4. Run `load.py` to create case lists
...
Download the expression data and generic assay files from Synapse
```
python3 lens_files.py
  --dataset Riaz \
  --generic-assay-data-synid syn12345678 \
  --generic-assay-metadata-synid syn23456789 \
  --expression-data-synid syn34567890 \
  --expression-metadata-synid syn45678901 \
  --datahub_tools_path /<some_path>/datahub-study-curation-tools 
```
